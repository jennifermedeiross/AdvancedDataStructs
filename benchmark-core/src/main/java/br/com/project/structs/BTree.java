...
/**
 * Implementação de uma B-Tree (Árvore B) para armazenar objetos da classe {@code Pessoa}.
 * A árvore mantém os dados ordenados pelo CPF e suporta inserção e busca eficiente.
 */
public class BTree {
...
private void delete(BTreeNode node, String cpf) {
    ...
    if (idx < node.keys.size() && cpf.equals(node.keys.get(idx).getCpf())) {
        ...
        if (node.isLeaf) return; // Se for folha e não achou, CPF não está na árvore

        boolean lastChild = (idx == node.keys.size());

        // Se o filho onde devemos procurar tiver menos que 't' chaves, garantimos que ele terá no mínimo 't'
...
private void deleteInternalNode(BTreeNode node, int idx) {
    ...
    // Caso 1: Se o filho anterior (esquerda) tem pelo menos 't' chaves, substituímos pela maior chave do filho esquerdo
    if (node.children.get(idx).keys.size() >= t) {
        Pessoa predecessor = getPredecessor(node, idx);
        node.keys.set(idx, predecessor);
        delete(node.children.get(idx), predecessor.getCpf());
    }
    // Caso 2: Se o filho direito tem pelo menos 't' chaves, substituímos pela menor chave do filho direito
    ...
    // Caso 3: Ambos os filhos têm menos que 't' chaves, então fundimos os dois filhos e removemos a chave
    else {
        merge(node, idx);
